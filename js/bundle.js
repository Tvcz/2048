...
class Grid {
    constructor(size, previousState) {
        ...
        this.tiles = [];
        if (previousState) {
            this.buildFromState(previousState);
        }
    }
    ...
    cellContent(position) {
        if (this.withinBounds(position)) {
            return this.cells.get(position) || null;
        }
        else {
            return null;
        }
    ...
    insertTile(tile) {
        ...
        this.cells.set(tile.position, tile);
    ...
    removeTile(tile) {
        this.cells.delete(tile.position);
    ...
    serialize() {
        ...
        for (let x = 0; x < this.size; x++) {
            let row = [];
            for (let y = 0; y < this.size; y++) {
                row.push(this.cellContent(new Position(x, y)));
            }
            cellState.push(row);
        }
        return {
